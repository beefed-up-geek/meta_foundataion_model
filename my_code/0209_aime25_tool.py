"""
AIME25 Tool-Augmented Solver - ReAct Agent with Math Tools
Uses models.py for LLM instances with LangGraph ReAct Agent and math tools.

Usage:
    python 0209_aime25_tool.py --model gemini-3-flash
    python 0209_aime25_tool.py --model gpt-5 --num_problems 10 --reasoning
"""
...
from environment.aime25.agent_tools.tools import math_tools
...
For AIME problems, the answer is always an integer from 0 to 999.
"""
...
def main():
    parser = argparse.ArgumentParser(description='AIME25 Problem Solver (ReAct Agent with Tools)')
    ...
    # Load dataset
    print(f"\n{'='*60}")
    print(f"AIME25 Problem Solver (LangGraph ReAct Agent with Tools)")
    print(f"{'='*60}")
    ...
    output_file = os.path.join(output_dir, f'aime25_tool_{model_name}{model_suffix}_{timestamp}.json')
